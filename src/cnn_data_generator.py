...
class DataGenerator:
    ...
    def plot_distribution(self, is_validation=False):
        ...
        counts = {piece: 0 for piece in pieces}
        subfolder = "validation" if is_validation else "train"
        for piece in pieces:
            folder_path = f"../data/cnn/{subfolder}/{piece}"
            counts[piece] = len(os.listdir(folder_path))
            print(f"{piece}: {counts[piece]}")

        plt.bar(counts.keys(), counts.values())
        plt.show()

    def move_from_folder(self, folder_path):
        folder_path = format_path(folder_path)
        for file in os.listdir(folder_path):
            value = file.split('.')[0]
            source_path = os.path.join(folder_path, file)
            destination_path = f"../data/cnn/train/{value}/template_{file}"
